...
{
	char lineBuffer[32];
	uint16_t batteryDeciV;
	PID_SpeedConfig_t speedProfile;

	PID_GetSpeedConfig(&speedProfile);
	batteryDeciV = ADC_Bat_GetVoltageDeciV();

	OLED_ClearArea(0, 0, 128, 64);

	sprintf(lineBuffer, "%s CL%u",
			Motor_GetCarStateName(Motor_GetCarState()),
			(unsigned int)Motor_IsClosedLoopEnabled());
	OLED_ShowString(0, 0, lineBuffer, OLED_6X8);

	sprintf(lineBuffer, "SP%.2f %.2f %.2f",
			speedProfile.kp,
			speedProfile.ki,
			speedProfile.kd);
	OLED_ShowString(0, 8, lineBuffer, OLED_6X8);

	sprintf(lineBuffer, "L%.0f O%.0f",
			speedProfile.integralLimit,
			speedProfile.outputLimit);
	OLED_ShowString(0, 16, lineBuffer, OLED_6X8);

	sprintf(lineBuffer, "LF T%4ld A%4ld U%+3d%%",
			(long)Motor_GetTargetSpeedRpm((Motor_Id_t)BOARD_CAR_LEFT_FRONT_MOTOR),
			(long)Motor_GetActualSpeedRpm((Motor_Id_t)BOARD_CAR_LEFT_FRONT_MOTOR),
			Motor_GetOutputPercent((Motor_Id_t)BOARD_CAR_LEFT_FRONT_MOTOR));
	OLED_ShowString(0, 24, lineBuffer, OLED_6X8);

	sprintf(lineBuffer, "RF T%4ld A%4ld U%+3d%%",
			(long)Motor_GetTargetSpeedRpm((Motor_Id_t)BOARD_CAR_RIGHT_FRONT_MOTOR),
			(long)Motor_GetActualSpeedRpm((Motor_Id_t)BOARD_CAR_RIGHT_FRONT_MOTOR),
			Motor_GetOutputPercent((Motor_Id_t)BOARD_CAR_RIGHT_FRONT_MOTOR));
	OLED_ShowString(0, 32, lineBuffer, OLED_6X8);

	sprintf(lineBuffer, "LR T%4ld A%4ld U%+3d%%",
			(long)Motor_GetTargetSpeedRpm((Motor_Id_t)BOARD_CAR_LEFT_REAR_MOTOR),
			(long)Motor_GetActualSpeedRpm((Motor_Id_t)BOARD_CAR_LEFT_REAR_MOTOR),
			Motor_GetOutputPercent((Motor_Id_t)BOARD_CAR_LEFT_REAR_MOTOR));
	OLED_ShowString(0, 40, lineBuffer, OLED_6X8);

	sprintf(lineBuffer, "RR T%4ld A%4ld U%+3d%%",
			(long)Motor_GetTargetSpeedRpm((Motor_Id_t)BOARD_CAR_RIGHT_REAR_MOTOR),
			(long)Motor_GetActualSpeedRpm((Motor_Id_t)BOARD_CAR_RIGHT_REAR_MOTOR),
			Motor_GetOutputPercent((Motor_Id_t)BOARD_CAR_RIGHT_REAR_MOTOR));
	OLED_ShowString(0, 48, lineBuffer, OLED_6X8);

	sprintf(lineBuffer, "BAT%u.%u Y%+03d G%+03d",
			(unsigned int)(batteryDeciV / 10U),
			(unsigned int)(batteryDeciV % 10U),
			(int)MPU6050_GetYawDeg(),
			(int)MPU6050_GetGyroZDps());
	OLED_ShowString(0, 56, lineBuffer, OLED_6X8);
...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏模式*/
	OLED_GPIO_ClockCmd(OLED_SCL_PORT, ENABLE);
